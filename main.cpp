...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
#define MAX_FRAME 1700//1700
#define MIN_NUM_FEAT 2000

vector<int> lcd;
// number of training images
const int NIMAGES =1700; //1700;//max 4541;

const static char* dataset_images_location = "/home/psh0302/RAIL/week5/data_odometry_gray/dataset/sequences/00/image_0";
const static char* dataset_poses_location = "/home/psh0302/RAIL/week5/data_odometry_poses/dataset/poses/00.txt";
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
int main(int argc, char** argv)
{
  ...
  for(int numFrame=2; numFrame < MAX_FRAME; numFrame++) {
    ...
    //cout << numFrame << " : " <<IN_LCD(numFrame) <<endl; //debuging

    if(IN_LCD(numFrame)==1) circle(traj, cv::Point(x, y), 1, CV_RGB(0, 0, 255), 2);
    else circle(traj, cv::Point(x, y), 1, CV_RGB(255, 0, 0), 2);
    circle(traj, cv::Point(groundPoses[numFrame].x+600, groundPoses[numFrame].y+100), 1, CV_RGB(0, 255, 0), 2);

    rectangle(traj, cv::Point(10, 30), cv::Point(550, 50), CV_RGB(0, 0, 0), -1); //CV_FILLED -> -1 replace for OpenCV4.x.x
...
void loadFeatures(vector<vector<cv::Mat > > &features)
{
  ...
  {
        char filename1[200];
    sprintf(filename1, "%s/%06d.png", dataset_images_location, i);
